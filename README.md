...
`snailmail setup` only creates repositories; it refuses a name that is already
configured, and there is no command that adds a signing key to an existing one.
Until there is, edit `snailmail.toml` directly and add both lines to the
repository's table:

```toml
signing_keys = ['archive-signing']
signing_keyring = 'keys/archive-signing.gpg'
```

`snailmail keys audit` verifies the result.
